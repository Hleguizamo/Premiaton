...
class cargarEmpleadosCommand extends ContainerAwareCommand
	{
	public $conn;
    protected function configure() {
        parent::configure();
        $this->setName('actualiza:empleados')->setDescription('Actualizar Empleados');		
    }

    protected function execute(InputInterface $input, OutputInterface $output) {
		
        $output->writeln("Inicia la carga de Empleados");    

        $terceros = "/home/planos/terceros.txt";
		$drog = fopen($terceros,"rb");
		$y = 0;
		$z = 0;
		$codigos = "";
		...
		$this->processLogAction("0","Cron", "Carga Empleados el Cron Total: $z - Codigos: ($codigos)");	
